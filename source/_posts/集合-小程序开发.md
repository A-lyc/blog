...
注意：
- 小程序的云函数和 uni-app 的云函数是两个不同的平台

优点：
- 微信的云函数拥有天然鉴权，相对于自己的服务器会少写些逻辑代码
...
> 同时支持 h5 和 小程序
> 使用 json 配置的方式就可以拼合生成图片
> 拼合时具体内容的位置去 ps 中量 x 和 y 坐标即可
> 非常方便

坑：
- uni-app 项目使用
> 若项目为 uni-app 则需要先 clone 原项目
> 用小程序开发者工具打开 example/weapp 目录
> 构件 miniprogram_npm 目录
> 复制 miniprogram_npm 到 uni-app 项目中的 wxcomponents 目录（名称依旧用 miniprogram_npm，改了会报错）
> 再在 uni-app 项目 src/pages.json 中注册组件 json2canvas": "/wxcomponents/miniprogram_npm/json2canvas/index
> 之后照常当时 vue 组件使用即可
